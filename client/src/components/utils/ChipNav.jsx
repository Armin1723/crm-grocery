...
const ChipNav = ({ chips, baseUrl = "/" }) => {
  ...
  return (
    <div className="flex gap-2 text-xs md:text-sm justify-start items-start p-3 bg-[var(--color-primary)] h-fit w-full select-none">
      <div className="wrapper flex gap-2 justify-start items-start w-full overflow-x-auto">
        {/* Leading chip for going back */}
        {canGoBack && (
          <button
            onClick={() => navigate(-1)} // Go back to the previous page
            className="px-3 py-1.5 bg-[var(--color-sidebar)] rounded-full flex items-center gap-2 h-fit w-fit hover:bg-accent/10 border border-neutral-500/30 transition-all duration-300 ease-in"
          >
