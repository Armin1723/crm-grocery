...
const ViewCustomers = () => {
  ...
  const { data: results, isFetching: loading } = useQuery({
    queryKey: ["customers", { sort, sortType, limit, page, query }],
    queryFn: async () => {
      const response = await fetch(
        `${
          import.meta.env.VITE_BACKEND_URL
        }/api/v1/customers?sort=${sort}&sortType=${sortType}&limit=${limit}&query=${query}&page=${page}`,
        {
          credentials: "include",
        }
      );
    ...
    <div className="p-3 rounded-md flex h-full flex-col gap-2 border border-neutral-500/50 bg-[var(--color-sidebar)]">
      <div className="top flex w-full justify-between items-center flex-wrap my-2">
        <div className="title flex items-center gap-2 flex-wrap ">
          <p className="text-xl max-lg:text-lg font-bold ">View Customers</p>
          <p
            className={`${
              ...
              results?.customers?.map((customer, index) => {
                return (
                  ...
                  >
                    ...
                    >
                      {customer.balance > 0 && <FaArrowDown />}
                      {customer.balance < 0 && <FaArrowUp />}
