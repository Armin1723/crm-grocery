...
const addCompany = async (req, res) => {
  ...
  if (existingInitials) {
    return res.status(400).json({
      message: "Initials already exists",
      success: false,
      errors: {
  ...
  process.nextTick(async () => {
    await sendMail(
      ...
      companyRegistrationMailTemplate(populatedUser, company)
    );
  });
};
...
const updateCompany = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Company updated successfully", user });
};
